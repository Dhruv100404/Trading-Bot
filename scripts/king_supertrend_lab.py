...
def write_report(
    ...
    lines = [
        ...
        "## No-lookahead audit",
        "",
        "- Weekly Supertrend signals are evaluated only after the weekly candle is complete; entries are always the next week's open.",
        "- King Candle signals are evaluated only after the King candle week is complete; trigger checks start from later weekly candles.",
        "- The King Candle trigger window checks only Supertrend direction from already completed weeks and does not use the trigger week's final Supertrend state before entry.",
        "- King Candle targets are not credited inside the same weekly candle that triggers the entry because weekly OHLC cannot prove the sequence after the stop-style fill.",
        "- Supertrend trailing exits are acted on at the next week's open after a completed weekly Supertrend negative state.",
        "",
        "## Summary",
        "",
